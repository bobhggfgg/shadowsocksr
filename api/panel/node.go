...
type ShadowsocksRNode struct {
	...
	ProtocolSettingsBack json.RawMessage `json:"protocolSettings"`
	Obfs                 string          `json:"obfs"`
	ObfsSettings         json.RawMessage `json:"obfs_settings"`
	ObfsSettingsBack     json.RawMessage `json:"obfsSettings"`
}
...
type tidalabConfigResponse struct {
	Msg  string          `json:"msg"`
...
func (c *Client) GetNodeInfo() (node *NodeInfo, err error) {
	...
	case "shadowsocks":
		...
		node.Security = None
	case "shadowsocksr":
		rsp := &ShadowsocksRNode{}
		body := r.Body()
		if c.NodeType == "shadowsocksr" {
			wrapper := &tidalabConfigResponse{}
			if err = json.Unmarshal(body, wrapper); err != nil {
				return nil, fmt.Errorf("decode shadowsocksr wrapper error: %s", err)
			}
			body = wrapper.Data
		}
		err = json.Unmarshal(body, rsp)
		if err != nil {
			return nil, fmt.Errorf("decode shadowsocksr params error: %s", err)
		}
		if len(rsp.ProtocolSettingsBack) > 0 {
			rsp.ProtocolSettings = rsp.ProtocolSettingsBack
			rsp.ProtocolSettingsBack = nil
		}
		if len(rsp.ObfsSettingsBack) > 0 {
			rsp.ObfsSettings = rsp.ObfsSettingsBack
			rsp.ObfsSettingsBack = nil
		}
		cm = &rsp.CommonNode
		node.ShadowsocksR = rsp
		node.Security = None
	case "trojan":
		rsp := &TrojanNode{}
		err = json.Unmarshal(r.Body(), rsp)
